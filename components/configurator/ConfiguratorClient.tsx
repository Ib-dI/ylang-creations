...
import ProductPreviewPanel, {
  type ProductPreviewPanelHandle,
} from "@/components/configurator/ProductPreviewPanel";
...
const ConfiguratorClient = ({
  ...
  useEffect(() => {
    const path = initialProductId
      ? `${pathname}?product=${initialProductId}`
      : pathname;
  ...
  const [configuration, setConfiguration] =
    useState<ConfigurateurConfiguration>(() => {
      ...
      return {
        ...
        fabric: fabrics[0],
        size:
          initialConfigProduct?.defaultSize ??
          initialConfigProduct?.sizes?.[0] ??
          null,
        embroideries: [""],
        embroideryColor: embroideryColors[0]?.hex ?? "#D4AF37",
        embroideryFont: embroideryFonts[0] ?? FALLBACK_EMBROIDERY_FONT,
        selectedColor:
          initialConfigProduct?.colorMaskImage && productColors[0]
            ? productColors[0].hex
            : null,
      };
    });
  ...
  const handleSelectProductColor = (hex: string) =>
    setConfiguration((prev) => ({ ...prev, selectedColor: hex }));
  const handleSelectFabric = (fabric: ConfigurateurFabric) =>
  ...
  const handleRemoveText = (index: number) =>
    setConfiguration((prev) => ({
      ...prev,
      embroideries: prev.embroideries.filter((_, i) => i !== index),
    }));
      ...
      configuration: {
        ...
        embroidery: configuration.embroideryFont
          ? configuration.embroideries
              .filter(Boolean)
              .map((t) => normalizeForFont(t, configuration.embroideryFont!.id))
              .join(", ") || undefined
        ...
        embroideryColorName:
          ...
          configuration.embroideryFont?.supportsThreadColor !== false
            ...
            : undefined,
        embroideryFont: configuration.embroideries.some((e) => e)
          ? (configuration.embroideryFont?.name ?? undefined)
          : undefined,
        ...
        selectedColorName: configuration.selectedColor
          ? (productColors.find((c) => c.hex === configuration.selectedColor)
              ?.name ?? undefined)
          : undefined,
      },
              ...
              {tabs.map((tab, index) => {
                ...
                return (
                  <button
                    ...
                    disabled={isLocked}
                    className="shrink-0 pb-3 transition-[border-color,color,opacity] duration-200"
                    style={{
                  ...
                  >
                    ...
                    <span className="font-body ml-1.5 hidden text-sm sm:inline">
                      {tab.label}
                    </span>
                  </button>
                );
