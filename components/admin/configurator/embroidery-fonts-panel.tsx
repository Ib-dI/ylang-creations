...
const AVAILABLE_FONT_DEFINITIONS: Array<
  Pick<
    EmbroideryFont,
    "id" | "name" | "folder" | "format" | "supportsThreadColor"
  >
> = [
  {
    id: "moonlight",
...
export default function EmbroideryFontsPanel() {
  ...
  const [editingFont, setEditingFont] =
    useState<Partial<EmbroideryFont> | null>(null);
  ...
  const toggleActive = async (font: EmbroideryFont) => {
    try {
      const res = await fetch(
        `/api/admin/configurator/embroidery-fonts?id=${font.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(
        `/api/admin/configurator/embroidery-fonts?id=${id}`,
        { method: "DELETE" },
      );
      if (!res.ok) {
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <p className="font-body text-sm text-gray-500">
          Nom, prix et disponibilité des polices de broderie. Les fichiers et le
          calibrage restent gérés par le code.
        </p>
        <Button
          onClick={openCreateModal}
          disabled={unregisteredDefinitions.length === 0}
          className="gap-2"
        >
      ...
      {fonts.length === 0 ? (
        <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 bg-white py-24">
          ...
          <p className="font-body mt-1 text-sm text-gray-300">
            Ajoutez Moonlight et Alfabeto Liz pour commencer
          </p>
        </div>
      ) : (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-body mt-0.5 text-xs text-gray-400">
                    {font.folder} · .{font.format} ·{" "}
                    {(font.price / 100).toFixed(2)} €
                    {!font.supportsThreadColor &&
                      " · Couleurs natives (fil non personnalisable)"}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    onClick={() => toggleActive(font)}
                    className={`font-body px-2 py-1 text-[10px] font-semibold uppercase ${
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Nom, prix et disponibilité affichés au client.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            {!fonts.some((f) => f.id === editingFont?.id) && (
              <div>
                <label className="font-body mb-1 block text-xs font-medium text-gray-600">
                  Police disponible
                </label>
                <select
                  value={editingFont?.id || ""}
                  onChange={(e) => {
                    const def = AVAILABLE_FONT_DEFINITIONS.find(
                      (d) => d.id === e.target.value,
                    );
                    if (def) setEditingFont((prev) => ({ ...prev, ...def }));
                  }}
                  className="font-body w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
                >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingFont((prev) => ({ ...prev!, name: e.target.value }))
                }
                className="font-body w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
              />
            </div>
            ...
            >
              <span
                className={`absolute top-1 left-1 h-4 w-4 rounded-full bg-white shadow transition-transform duration-200 ease-[var(--ease-out)] ${editingFont?.isActive ? "translate-x-5" : "translate-x-0"}`}
              />
            </button>
          </div>
